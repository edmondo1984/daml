...
final class LedgerTestCasesRunner(
  ...
  private def prepareResourcesAndRun(implicit
    ...
    val resources =
      for {
        actorSystem <- ResourceOwner.forActorSystem(createActorSystem).acquire()
        materializer <- ResourceOwner.forMaterializer(actorSystem).acquire()
      } yield (materializer, participants)
    ...
    val results =
      for {
        (materializer, participants) <- resources.asFuture
        results <- run(participants)(materializer, materializer.executionContext)
      } yield results

    results.onComplete(_ => resources.release())
